...
const observation = z.number().int().min(0).max(10_000_000)
...
const requireDefinedEntries = (value: Record<string, unknown>, context: z.RefinementCtx) => {
  for (const [key, item] of Object.entries(value)) {
    if (item === undefined) context.addIssue({code: z.ZodIssueCode.custom, path: [key], message: "Omit unselected fields instead of setting them to undefined."})
  }
}
...
export const RegressionSummarySchema = z.object({
  ...
  distribution: z.discriminatedUnion("method", [
    ...
    z.object({
      ...
      baseline_shots: z.number().int().min(1).max(10_000_000), candidate_shots: z.number().int().min(1).max(10_000_000),
    }).strict(),
  ]).nullable(),
...
export type RegressionCheck = {
  ...
  baseline?: number; candidate?: number; maximum?: number
  estimate?: number; lower?: number; upper?: number
}

export function parseRegressionPolicy(input: unknown): RegressionPolicy {
...
export function inspectRegressionSummary(input: unknown): {summary: RegressionSummary; checks: RegressionCheck[]} {
  ...
  if (summary.changed_fields.includes("case_id")) verdict = "INCOMPATIBLE"
  else if (summary.baseline_status !== "EXECUTED") verdict = summary.baseline_status
  else if (summary.candidate_status !== "EXECUTED") verdict = summary.candidate_status
  else if (summary.changed_fields.some(x => !permitted.has(x))) verdict = "INCOMPATIBLE"
  ...
  if (!verdict) {
    for (const metric of Object.keys(policy.resources) as (keyof RegressionPolicy["resources"])[]) {
      ...
      if (baseline === undefined || candidate === undefined) checks.push({metric, verdict: "INCONCLUSIVE"})
      else {
    ...
    if (policy.max_total_variation !== null) {
      ...
      if (!d) checks.push({metric: "total_variation", verdict: "INCONCLUSIVE"})
      else {
        let radius = 0
        if (d.method === "EXACT_IDEAL") {
          if (d.baseline_shots !== null || d.candidate_shots !== null) throw new Error("Exact probabilities have no shot budget.")
        } else {
          if (!d.baseline_shots || !d.candidate_shots) throw new Error("Sampled observations require both shot budgets.")
    ...
    } else if (summary.distribution !== null) throw new Error("Unselected distribution must be redacted.")
    for (const values of [summary.resources.baseline, summary.resources.candidate]) {
      if (values && Object.keys(values).some(x => !(x in policy.resources))) throw new Error("Unselected resources must be redacted.")
    }
